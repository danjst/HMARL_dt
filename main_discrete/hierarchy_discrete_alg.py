...
import sys
# import wandb
# wandb.init(project='Main_Discrete')
...
#         self.target_entropy = -tf.constant(l_action, dtype=tf.float32)
    ...
    def create_networks(self):
        ...
        self.probs = 0.9 * probs + 0.1/float(self.l_action)
...
#             obs_next = obs_next.reshape((-1,self.hidden_factor)) # None * K
        ...
        ##########################################
        if iv_action_only:
            actions = (act_index/action_level).astype(int)
        else:
            actions = (act_index%action_level).astype(int)
...
#             self.probs1 = self.lambda_A * (1-self.lambda_A) + 0.1/float(self.l_action)
...
#         iv_actions = np.array(actions1).reshape((-1, self.n_agents))[:,0]
#         vaso_actions = np.array(actions2).reshape((-1, self.n_agents))[:,1]
...
#         self.target_entropy = -tf.constant(l_action, dtype=tf.float32)
    ...
    def train_step(self, i, use_FM, writer=None):
        ...
        act_index = eval_act_index.reshape((eval_act_index.shape[0], 1))
        ##########################################
        a_0 = (act_index/action_level).astype(int)
        a_1 = (act_index%action_level).astype(int)
        actions = np.concatenate((a_0, a_1), axis = 1)
